...
def test_dicom_to_dimble(dicom_file: Path, benchmark):
    dimble_file = Path("/tmp") / dicom_file.with_suffix(".dimble").name
    dimble.dicom_to_dimble(dicom_file, dimble_file)

    benchmark(dimble.dicom_to_dimble, dicom_file, dimble_file)


@pytest.mark.parametrize("dicom_file", dicom_files, ids=dicom_files_ids)
def test_load_dimble(dicom_file: Path, benchmark):
    dimble_file = Path("/tmp") / dicom_file.with_suffix(".dimble").name
    dimble.dicom_to_dimble(dicom_file, dimble_file)
...
@pytest.mark.parametrize("dicom_file", dicom_files, ids=dicom_files_ids)
def test_dimble_to_dicom(dicom_file: Path, benchmark):
    dimble_file = Path("/tmp") / dicom_file.with_suffix(".dimble").name
    dimble.dicom_to_dimble(dicom_file, dimble_file)
    ...
    benchmark(dimble.dimble_to_dicom, dimble_file, reconstructed_dicom_file)
